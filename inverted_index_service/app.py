...
#     file_name: str = Query("tot_docs_clean.csv"),
#     output_file: str = Query("inverted_index.json"),
...
#         return {"error": f"❌ لم يتم العثور على الملف: {file_path}"}

#     df = pd.read_csv(file_path)
#     if "doc_id" not in df.columns or text_column not in df.columns:
...
#         tokens = clean_text(str(row[text_column]))
#         for term in tokens:
#             index[term].add(doc_id)

#     # تحويل القيم إلى قائمة بدل set
#     index = {term: list(docs) for term, docs in index.items()}

#     with open(output_path, "w", encoding="utf-8") as f:
#         json.dump(index, f, indent=4)

#     return {
#         "✅": "تم بناء فهرس معكوس Inverted Index",
#         "terms": len(index),
#         "saved_to": output_path
#     }

# @app.get("/search-inverted-index")
# def search_inverted_index(
#     dataset: str = Query("trec_tot"),
#     index_file: str = Query("inverted_index.json"),
#     term: str = Query(...)
# ):
#     base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'data', dataset))
...
#         return {"error": f"❌ ملف الفهرس غير موجود: {index_path}"}

#     with open(index_path, "r", encoding="utf-8") as f:
#         index = json.load(f)

#     # ✅ تنظيف المصطلح بنفس أسلوب الفهرسة
#     clean_term = stemmer.stem(lemmatizer.lemmatize(term.lower()))
#     doc_ids = index.get(clean_term, [])

#     return {
#         "term": clean_term,
#         "found_in_documents": doc_ids,
#         "count": len(doc_ids)
#     }
import sqlite3
from fastapi import FastAPI, Query
